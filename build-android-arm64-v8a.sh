...
if [ -z $SHERPA_ONNX_ENABLE_NNAPI ]; then
  SHERPA_ONNX_ENABLE_NNAPI=OFF
fi

if [ -z $SHERPA_ONNX_ENABLE_QNN ]; then
  SHERPA_ONNX_ENABLE_QNN=ON
fi

cmake -DCMAKE_TOOLCHAIN_FILE="$ANDROID_NDK/build/cmake/android-legacy.toolchain.cmake" \
    ...
    -DSHERPA_ONNX_ENABLE_NNAPI=$SHERPA_ONNX_ENABLE_NNAPI \
